...
fn listen_usn(listener: UsnVolumeListener, tx: Sender<Value>) -> Result<(), WinThingError> {
    let live_volume = WindowsLiveNtfs::from_volume_path(&listener.source)?;
    ...
    if listener.config.enumerate_paths {
        ...
        mapping = Some(live_volume.get_folder_mapping());
    ...
    loop {
        ...
        let entry_meta = EntryMeta::new(&listener.source, 0);
                ...
                Some(ref mut mapping) => {
                    ...
                    if file_attributes.contains(flags::FileAttributes::FILE_ATTRIBUTE_DIRECTORY) {
                        if reason_code.contains(flags::Reason::USN_REASON_RENAME_OLD_NAME) {
                            ...
                            mapping.remove_mapping(file_ref);
                            ...
                            if listener.config.historical_flag && entry_usn < catch_up_usn {
                                mapping.add_mapping(file_ref, file_name.clone(), parent_ref)
                            } else {
                                mapping.remove_mapping(file_ref);
                            }
                    ...
                    let full_path =
                        match mapping.enumerate_path(parent_ref.entry, parent_ref.sequence) {
                            Some(path) => path,
...
impl UsnListenerConfig {
    ...
    pub fn get_listener(self, volume: &str) -> UsnVolumeListener {
        UsnVolumeListener::new(volume.to_string(), self)
    }
}
...
impl UsnVolumeListener {
    pub fn new(source: String, config: UsnListenerConfig) -> Self {
        Self { source, config }
    }
