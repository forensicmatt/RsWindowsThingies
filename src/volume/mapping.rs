...
impl FolderMapping {
    ...
    pub fn add_mapping(
        ...
        self.cache.pop(&entry_reference);

        self.mapping.insert(entry_reference, entry_map);
    }
    ...
    fn enumerate_path_queue(&self, lookup_ref: &MftReference, path_queue: &mut Vec<String>) {
        if lookup_ref.entry != 5 {
            match self.mapping.get(&lookup_ref) {
                Some(folder_map) => {
                    self.enumerate_path_queue(&folder_map.parent, path_queue);
    ...
    pub fn enumerate_path(&mut self, entry: u64, sequence: u16) -> Option<String> {
        let lookup_ref = MftReference { entry, sequence };
        ...
        match self.cache.get_mut(&lookup_ref) {
            ...
            None => {
                ...
                self.enumerate_path_queue(&lookup_ref, &mut path_queue);

                let full_path = path_queue.join("/");

                self.cache.put(lookup_ref, full_path.clone());
