...
impl ChannelConfig {
    pub fn new(channel: String) -> Result<Self, WinThingError> {
        let handle = evt_open_channel_config(&channel)?;
    ...
    pub fn get_config_isolation(&self) -> Option<u64> {
        match evt_get_channel_config_property(&self.handle, EvtChannelConfigIsolation) {
            Some(v) => match v.get_variant_value() {
                Ok(variant_value) => match variant_value {
                ...
                Err(e) => {
                    error!(
                        "Error getting variant value for EvtChannelConfigIsolation: {:?}",
                        e
                    );
                }
            },
    ...
    pub fn get_log_file_path(&self) -> Option<String> {
        match evt_get_channel_config_property(&self.handle, EvtChannelLoggingConfigLogFilePath) {
            Some(v) => match v.get_variant_value() {
                Ok(variant_value) => match variant_value {
                ...
                Err(e) => {
                    error!(
                        "Error getting variant value for EvtChannelConfigClassicEventlog: {:?}",
                        e
                    );
                }
            },
    ...
    pub fn get_config_type(&self) -> Option<u64> {
        match evt_get_channel_config_property(&self.handle, EvtChannelConfigType) {
            Some(v) => match v.get_variant_value() {
                Ok(variant_value) => match variant_value {
                    VariantValue::UInt(i) => return Some(i),
                    other => {
                ...
                Err(e) => {
                    error!(
                        "Error getting variant value for EvtChannelConfigType: {:?}",
                        e
                    );
                }
            },
    ...
    pub fn is_classic_event_log(&self) -> bool {
        match evt_get_channel_config_property(&self.handle, EvtChannelConfigClassicEventlog) {
            Some(v) => match v.get_variant_value() {
                Ok(variant_value) => match variant_value {
    ...
    pub fn is_enabled(&self) -> bool {
        match evt_get_channel_config_property(&self.handle, EvtChannelConfigEnabled) {
            Some(v) => match v.get_variant_value() {
                Ok(variant_value) => match variant_value {
                    VariantValue::Boolean(b) => b,
                    other => {
                ...
                Err(e) => {
                    error!(
                        "Error getting variant value for EvtChannelConfigEnabled: {:?}",
                        e
                    );
                    false
                }
    ...
    pub fn to_json_value(&self) -> Result<Value, WinThingError> {
        ...
        for (key, id) in &CHANNEL_PROPERTIES {
            let variant = match evt_get_channel_config_property(&self.handle, *id) {
                Some(v) => v,
                None => continue,
            };
...
pub fn get_channel_name_list(session: &Option<EvtHandle>) -> Result<Vec<String>, WinThingError> {
    let mut channel_name_list: Vec<String> = Vec::new();

    let channel_enum_handle = evt_open_channel_enum(session)?;
...
///   DWORD      Flags
/// );
pub fn evt_open_channel_enum(session: &Option<EvtHandle>) -> Result<EvtHandle, WinThingError> {
    let session = match session {
        Some(s) => s.0,
        None => null_mut(),
    };

    let enum_handle = unsafe { EvtOpenChannelEnum(session, 0) };
...
/// );
fn evt_get_channel_config_property(
    evt_handle: &EvtHandle,
    property_id: EVT_CHANNEL_CONFIG_PROPERTY_ID,
) -> Option<EvtVariant> {
        ...
        if last_error == ERROR_INSUFFICIENT_BUFFER {
            ...
            match unsafe {
                ...
                _ => {
                    ...
                    return Some(EvtVariant(variant));
                }
            }
        }
    }
...
fn evt_open_channel_config(channel_path: &String) -> Result<EvtHandle, WinThingError> {
    ...
    let result = unsafe { EvtOpenChannelConfig(null_mut(), channel_path_u16.as_ptr(), 0) };
    if result.is_null() {
        let last_error = unsafe { GetLastError() };

        let message = format!(
            "EvtOpenChannelConfig('{}') failed with code {}",
            channel_path, last_error
        );

        Err(WinThingError::winapi_error(message))
    } else {
        Ok(EvtHandle(result))
    }
}
...
fn evt_next_channel_id(channel_enum_handle: EVT_HANDLE) -> Option<String> {
    ...
    let result =
        unsafe { EvtNextChannelPath(channel_enum_handle, 0, null_mut(), &mut buffer_used) };
    ...
    if result == 0 {
        ...
        if last_error == ERROR_INSUFFICIENT_BUFFER {
            ...
            match unsafe {
                ...
                _ => {
                    let channel_string = OsString::from_wide(&buffer[..(buffer.len() - 1)])
                        .to_string_lossy()
                        .to_string();
