...
impl VariantValue {
    ...
    pub fn to_json_value(&self) -> Value {
        match self {
            ...
            VariantValue::UInt(v) => Value::Number(Number::from(*v)),
            VariantValue::Int(v) => Value::Number(Number::from(*v)),
            VariantValue::Single(v) => match Number::from_f64(*v as f64) {
                Some(n) => Value::Number(n),
                None => {
                    eprintln!(
                        "Could not convert VariantValue::Single {} to Number from_f64.",
                        v
                    );
                    Value::Null
                }
            },
            VariantValue::Double(v) => match Number::from_f64(*v) {
                Some(n) => Value::Number(n),
                None => {
                    eprintln!(
                        "Could not convert VariantValue::Double {} to Number from_f64.",
                        v
                    );
                    Value::Null
                }
            ...
            VariantValue::Binary(v) => Value::String(hex::encode(&v)),
        }
    }
    ...
    pub fn from_variant(variant: &EVT_VARIANT) -> Result<Self, WinThingError> {
        ...
        let value = match variant.Type {
            ...
            EvtVarTypeAnsiString => {
                ...
                VariantValue::String(String::from_utf8(slice.to_vec())?)
            }
            EvtVarTypeSByte => {
                let val: &i8 = unsafe { variant.u.SByteVal() };
                VariantValue::Int(*val as i64)
            }
            EvtVarTypeByte => {
                let val: &u8 = unsafe { variant.u.ByteVal() };
                VariantValue::UInt(*val as u64)
            }
            EvtVarTypeInt16 => {
            ...
            EvtVarTypeBoolean => {
                ...
                VariantValue::Boolean(*val != 0)
            }
            EvtVarTypeBinary => {
                let val: u8 = unsafe { **variant.u.BinaryVal() };
                VariantValue::String(format!("{:02X}", val))
            }
            EvtVarTypeGuid => {
            ...
            unknown => {
                let message = format!(
                    ...
                    unknown,
                    variant.Count,
                    unsafe { std::ptr::read(&variant.u as *const _ as *const u64) }
                );

                return Err(WinThingError::unhandled_variant(message));
            }
        };
