...
pub extern "system" fn evt_subscribe_callback(
    ...
    let user_context: Box<&CallbackContext> = unsafe { Box::from_raw(user_context as *mut _) };

    match evt_render(event_handle) {
        Ok(xml_event) => {
            user_context.handle_record(xml_event);
        }
        Err(e) => {
...
pub fn register_event_callback(
    ...
    let context = Box::into_raw(Box::from(context));
    ...
    Ok(EvtHandle(subscription_handle))
}
